...
class IntroScene(LinearTransformationScene):
    ...
    def construct(self):
        ...
        standard_basis_text = TextMobject("standard basis").to_corner(UP+RIGHT).set_color_by_gradient(*self.back_basis_cols)
        trans_basis_text = TextMobject("transformed basis").next_to(standard_basis_text,DOWN).shift(0.3*LEFT).set_color_by_gradient(*self.fore_basis_cols)
        ...
        goal = TextMobject("Goal",color=MAROON).scale(2).to_edge(UP)
        arrow = Vector(RIGHT,color=BLUE_D).rotate(axis=IN,about_point=ORIGIN,angle=PI/2).next_to(goal,DOWN)
        text = TextMobject("Visualize determinant of a composite matrix").set_color_by_gradient(
            self.det_col, *self.a_b_cols).set_width(FRAME_WIDTH-2).next_to(arrow,DOWN)
        goals_group = VGroup(goal,arrow,text)
        
        context = TexMobject("det(BA)", " = ", "det(B).det(A)").set_width(FRAME_WIDTH-2).next_to(text,2*DOWN)
        det_s = VGroup(context[0][:3], context[-1][:3],
                       context[-1][7:10]).set_color(self.det_col)
        ...
        back_img = ImageMobject("board2.jpg").set_width(FRAME_WIDTH)
        self.add(back_img)
        ...
        self.play(GrowFromPoint(fore_basis, ORIGIN),
                  GrowFromPoint(fore_sq, ORIGIN),FadeIn(fore_grid))
        ...
        grant = ImageMobject('3b1b.png').set_width(FRAME_WIDTH*0.9).set_height(FRAME_HEIGHT*0.7)
        eola = TextMobject("Essence Of Linear Algebra -","3Blue1Brown").set_width(FRAME_WIDTH-1).next_to(grant,UP)
...
class AreasGettingScaled(LinearTransformationScene):
    ...
    def construct(self):
        ...
        matrix = Matrix(np.array(self.t_matrix).transpose(), include_background_rectangle=True).scale(
            0.75).set_column_colors(*self.fore_basis_cols)
        a_matrix = VGroup(a_equals,matrix).arrange_submobjects().shift(UP+2*LEFT)
    
        self.play(Write(a_equals), FadeIn(
            matrix.background_rectangle), Write(matrix.brackets))
        self.wait()
        ...
        self.apply_transposed_matrix(self.t_i_matrix, run_time=1.2, added_anims=[
        ...
        self.apply_transposed_matrix(self.t_j_matrix, run_time=1.2, added_anims=[
                                     ShowCreationThenFadeAround(matrix.get_columns()[1])])
        ...
        self.play(ApplyMethod(a_matrix.to_corner,UP+LEFT),FadeOut(standard_basis_text),FadeOut(trans_basis_text))
        self.play(ApplyMethod(back_sq.shift,4*LEFT),ApplyMethod(back_basis.shift,4*LEFT))

        curved_arrow = CurvedArrow(back_sq.get_right()+0.2*RIGHT,fore_sq.get_left()+0.2*RIGHT,angle=-PI/4)
        question_mark = TextMobject("X ?").set_color(self.det_col).next_to(curved_arrow,UP)
        ...
        transformed_rect = Polygon(ORIGIN,[0.5,1,0],[1.5,1,0],[1,0,0],stroke_color=back_sq.get_stroke_color(),fill_color=back_sq.get_fill_color(),fill_opacity=back_sq.get_fill_opacity())
        transformed_rects = [transformed_rect,transformed_rect.copy().shift(RIGHT),transformed_rect.copy().shift(UP+0.5*RIGHT),transformed_rect.copy().shift(UP+1.5*RIGHT)]

        for index,rect in enumerate(transformed_rects):
            factor = TextMobject(f"{index+1}",color = self.det_col).scale(1.5).next_to(question_mark[0][0])
            self.play(TransformFromCopy(back_sq,rect),Transform(question_mark[0][1],factor))
            self.wait(0.5)

        self.wait()

        det_a = TexMobject("det(A)"," = ").scale(1.5).next_to(a_matrix,2*RIGHT)
...
class AnyAreaScalesBySameFactor(LinearTransformationScene):
    ...
    def construct(self):
        ...
        back_circ = Circle(fill_color=TEAL_E,fill_opacity=0.3).scale(0.75).move_to(2*LEFT+UP)
        fore_circ = back_circ.copy()
        back_vects = VGroup(Vector(RIGHT+DOWN),Vector(LEFT+DOWN))
        back_vect_rect = Polygon(ORIGIN,RIGHT+DOWN,2*DOWN,LEFT+DOWN,fill_color=TEAL_E,fill_opacity=0.3)
        fore_vects = back_vects.copy()
        fore_vect_rect = back_vect_rect.copy()

        self.add_background_mobject(back_grid,back_sq,back_basis,back_circ,back_vects,back_vect_rect)
        self.add_foreground_mobjects(fore_grid,fore_basis,fore_sq,fore_circ,fore_vects,fore_vect_rect)
        ...
        fade_out_mobs = [back_grid,fore_grid,fore_basis,back_basis,back_vects,fore_vects]
        ...
        arrow = Vector().rotate(axis=IN,angle=PI/2).next_to(back_sq,DOWN).scale(1.25)
        scale_factor = TextMobject("X 4").next_to(arrow)
...
class PropertyOfLinTrans(LinearTransformationScene):
    ...
    def construct(self):
        ...
        inherent_props = TextMobject("Linear Transformations : ").set_color(TEAL_E).to_corner(UP+LEFT)
        prop1 = TextMobject("1. Keep grid lines parallel and evenly spaced").scale(0.75)
        prop2 = TextMobject("2. Fix origin in place").scale(0.75)
        props = VGroup(prop1,prop2).arrange_submobjects(DOWN,aligned_edge=LEFT).next_to(inherent_props,2*DOWN,aligned_edge=LEFT)
...
class DeterminantOfMatrixB(LinearTransformationScene):
    ...
    def construct(self):
        ...
        standard_basis_text = TextMobject("standard basis").to_corner(
            UP+RIGHT).set_color_by_gradient(*self.back_basis_cols)
        trans_basis_text = TextMobject("transformed basis").next_to(
            standard_basis_text, DOWN).shift(0.3*LEFT).set_color_by_gradient(*self.fore_basis_cols)

        self.add(back_grid, back_basis, back_sq, standard_basis_text)
        self.wait()

        b_equals = TextMobject("B", "=")
        b_equals[0].set_color(self.a_b_cols[1]).scale(1.5)
        matrix = Matrix(np.array(self.t_matrix).transpose(), include_background_rectangle=True).scale(
            0.75).set_column_colors(*self.fore_basis_cols)
        b_matrix = VGroup(
            b_equals, matrix).arrange_submobjects().shift(UP+4*LEFT)

        self.play(Write(b_equals), FadeIn(
            matrix.background_rectangle), Write(matrix.brackets))
        self.wait()
        self.play(Write(matrix.get_columns()[0]))
        self.play(Write(matrix.get_columns()[1]))
        ...
        self.play(WiggleOutThenIn(back_basis[0]))
        self.add(fore_basis, fore_grid)
        self.apply_transposed_matrix(self.t_i_matrix, run_time=1, added_anims=[
                                     ShowCreationThenFadeAround(matrix.get_columns()[0], opacity=0.5)])
        self.remove(*self.moving_vectors, fore_grid)
        self.play(WiggleOutThenIn(back_basis[1]))
        self.add(fore_grid, fore_basis)
        self.apply_transposed_matrix(self.t_j_matrix, run_time=1, added_anims=[
                                     ShowCreationThenFadeAround(matrix.get_columns()[1])])
        self.play(Write(trans_basis_text, lag_ratio=0.3))
        self.wait()
        ...
        self.play(ApplyMethod(b_matrix.to_corner, UP+LEFT),
                  FadeOut(standard_basis_text), FadeOut(trans_basis_text),
                  ApplyMethod(back_sq.shift, 4*LEFT),
                  ApplyMethod(back_basis.shift, 4*LEFT)
                  )

        curved_arrow = CurvedArrow(
            back_sq.get_right()+0.2*RIGHT, fore_sq.get_left()+0.2*LEFT, angle=-PI/4)
        question_mark = TextMobject("X ?").set_color(
            self.det_col).next_to(curved_arrow, UP)
        question_mark[0][0].set_color(WHITE)
        scaled_area = VGroup(curved_arrow, question_mark)
        self.play(ShowCreation(scaled_area))
        ...
        transformed_rect = Polygon(ORIGIN, [-0.5, 0.5, 0], [1.5, 0.5, 0], [2, 0, 0], stroke_color=back_sq.get_stroke_color(
        ), fill_color=back_sq.get_fill_color(), fill_opacity=back_sq.get_fill_opacity())
        transformed_rects = [transformed_rect,
                             transformed_rect.copy().shift(0.5*(UP+LEFT))]

        for index, rect in enumerate(transformed_rects):
            factor = TextMobject(
                f"{index+1}", color=self.det_col).scale(1.5).next_to(question_mark[0][0]).add_background_rectangle()
            self.play(TransformFromCopy(back_sq, rect),
                      Transform(question_mark[0][1], factor))
            self.wait(0.5)

        det_b = TexMobject("det(B)", " = ").scale(
            1.5).next_to(b_matrix, 2*RIGHT)
...
class WhatMatrixMultipilcationIs(LinearTransformationScene):
    ...
    def construct(self):
        ...
        b_equals = TextMobject("B =").scale(1.25)
        b_equals[0][0].set_color(self.a_b_cols[1])
        b_mat = Matrix(self.b_mat.transpose(),include_background_rectangle=True).set_column_colors(*self.back_basis_cols).scale(.75)
        matrix_b = VGroup(b_equals,b_mat).arrange_submobjects().to_corner(UP+LEFT)
        
        a_equals = TextMobject("A =").scale(1.25)
        a_equals[0][0].set_color(self.a_b_cols[0])
        a_mat = Matrix(self.a_mat.transpose(),include_background_rectangle=True).set_column_colors(*self.fore_basis_cols).scale(.75)
        ...
        self.play(
            FadeOut(a_equals),FadeOut(b_equals),
            ApplyMethod(b_mat.to_corner,UP+LEFT)
        )
        self.play(ApplyMethod(a_mat.next_to,b_mat,buff=0.2))
        
        equals = TextMobject("=").next_to(a_mat)
        dummy_matrix = Matrix([[0,0],[0,0]],include_background_rectangle=True).scale(.75).next_to(equals)
        
        self.play(Write(equals),FadeIn(dummy_matrix.background_rectangle),Write(dummy_matrix.brackets))
        self.wait(2)

        #show transformation
        self.add_transformable_mobject(fore_grid)
        text1 = TextMobject(
            "Where do the basis vectors defined by A land on,"
            ).next_to(b_equals,2.5*DOWN, aligned_edge=LEFT).shift(3*DOWN).add_background_rectangle()
        text2 = TextMobject("during the transformation B ?").next_to(text1,DOWN,aligned_edge=LEFT).add_background_rectangle()
        ...
        self.apply_transposed_matrix(
            self.b_mat,run_time=2,
            added_anims=[Write(text2)]
            )
        self.wait()

        
        v1_landing_label = vector_coordinate_label(v1).next_to(v1)
        v2_landing_label = vector_coordinate_label(v2).next_to(v2,0.4*UP+0.5*RIGHT)
        ...
        v1_locs = [num.get_center() for num in [element for element in dummy_matrix.elements[::2]]]

        v2_land_numbers = [num for num in v2_landing_label.copy().elements]
        v2_locs = [num.get_center() for num in [element for element in dummy_matrix.elements[1::2]]]
        self.play(
            *[ApplyMethod(num.move_to,loc) for num,loc in zip(v1_land_numbers,v1_locs)],
            *[ApplyMethod(num.move_to,loc) for num,loc in zip(v2_land_numbers,v2_locs)],
            lag_ratio=1
        )
...
class ScaleFactorOfTransformingBasis(LinearTransformationScene):
    ...
    def construct(self):
        ...
        a_mat = Matrix(self.b_mat.transpose(),include_background_rectangle=True).scale(.75).to_corner(UP+LEFT,buff=0.3).set_column_colors(*self.fore_basis_cols)
        ...
        self.play(
            *[ApplyMethod(mob.fade,darkness=1) for mob in self.mobjects if mob not in [*back_squares,*fore_squares,*back_vects,*fore_vects]],
            
            *[ApplyMethod(mob.shift,4.5*LEFT+1.25*UP) for mob in [back_sq,*back_basis]],
            *[ApplyMethod(mob.shift,5.5*LEFT-2*UP) for mob in [fore_sq,*fore_basis]],

            *[ApplyMethod(mob.shift,RIGHT+1.25*UP) for mob in [back_mobs_copy[0], *back_mobs_copy[1]]],
            *[ApplyMethod(mob.shift,-2*UP) for mob in [fore_mobs_copy[0], *fore_mobs_copy[1]]]
        )
        self.wait()

        # show scale factors
        rect1 = Polygon(RIGHT+1.25*UP,1.75*UP+0.5*RIGHT,1.75*UP+2.5*RIGHT,3*RIGHT+1.25*UP).set_style(fill_opacity=back_sq.get_fill_opacity(),fill_color=back_sq.get_fill_color())
        trans_rects1 = [rect1, rect1.copy().shift(.5*UP+.5*LEFT)]
        
        factor = VGroup(Vector(1.5*RIGHT,color=WHITE).shift(2.5*LEFT+1.65*UP),TextMobject("X 2").move_to(2*LEFT+2*UP))
        factor[1][0][1].set_color(YELLOW)
        factor_copy = factor.copy().shift(3*DOWN)
        
        rect2 = Polygon(ORIGIN,2*RIGHT,2*RIGHT+2*DOWN,2*DOWN,fill_color=fore_sq.get_fill_color(),fill_opacity=fore_sq.get_fill_opacity())
        trans_rects2 = [rect2, rect2.copy().shift(2*RIGHT)]
        q_mark = TextMobject("X ?").next_to(factor_copy[0],UP)
        ...
        self.play(Transform(q_mark[0][-1],TextMobject("2",color=YELLOW).move_to(q_mark[0][1].get_center())))
        self.wait()
        ...
        self.play(*[FadeOut(mob) for mob in self.mobjects], run_time=2, lag_ratio=.3)
...
class FateOfUnitArea(LinearTransformationScene):
    ...
    def construct(self):
        ...
        b_mat = Matrix(self.b_mat.transpose(),include_background_rectangle=True).scale(.75).set_column_colors(*self.fore_basis_cols).to_corner(UP+LEFT)
        a_mat = Matrix(self.a_mat.transpose(), include_background_rectangle=True).scale(
            .75).set_column_colors(*self.fore_basis_cols).next_to(b_mat)
        b_text = TextMobject("B").next_to(b_mat,DOWN).set_color(self.a_b_cols[1])
        a_text = TextMobject("A").next_to(a_mat,DOWN).set_color(self.a_b_cols[0])
        det_a = TexMobject("det(A) = 4").next_to(b_mat,3.5*DOWN+0.1*RIGHT)
        det_b = TexMobject("det(B) = 2").next_to(det_a, DOWN)
        det_a[0][:3].set_color(self.det_col)
        ...
        self.add(back_grid, fore_grid,back_basis, back_sq,b_mat,b_text,a_mat,a_text,det_a,det_b)
        self.wait(4)
        self.play(ApplyWave(back_sq),amplitude=0.5)
        self.wait()
        
        self.apply_transposed_matrix(
            self.a_mat, run_time=1.5,
            added_anims = [Indicate(a_mat,scale_factor=1.1)]
            )
        self.wait(0.5)
        ...
        self.apply_transposed_matrix(self.b_mat, run_time=1.5,added_anims=[Indicate(b_mat,scale_factor=1.1)])
        self.wait()
        ...
        back_rect = Rectangle(width=FRAME_WIDTH,height=FRAME_HEIGHT,stroke_color=BLACK,fill_color=BLACK,fill_opacity=0.3)
        ...
        arrow_a = Arrow(back_sq.get_right()+0.05*RIGHT, back_sq_copy2.get_left()+0.05*LEFT)
        factor_a_copy = det_a[0][-1].copy().next_to(arrow_a,UP)
        cross_a = TextMobject("X").next_to(factor_a_copy,LEFT,buff=0.15).scale(.75)
        ...
        arrow_b = Arrow(back_sq_copy2.get_right(), back_sq_copy1.get_left()+0.05*LEFT)
        factor_b_copy = det_b[0][-1].copy().next_to(arrow_b, UP)
        cross_b = TextMobject("X").next_to(factor_b_copy,LEFT,buff=0.15).scale(.75)
        ...
        det_ab = TexMobject("det(BA) = det(B) X det(A)",include_background_rectangle=True).scale(1.3).move_to(3*RIGHT+2.5*UP)
        ...
        self.play(
            TransformFromCopy(factor_a_copy, factor_a_copy2)
        )
        self.play(Write(cross_copy))
        self.play(
            TransformFromCopy(factor_b_copy, factor_b_copy2)
        )
        self.wait(2)
        
        deter_a = TexMobject("det(A)").move_to(factor_a_copy2.get_center()).scale(1.3)
        deter_b = TexMobject("det(B)").move_to(factor_b_copy2.get_center()).scale(1.3)
        ...
        self.play(
            Transform(factor_a_copy2,deter_a),
            Transform(factor_b_copy2,deter_b)
        )
...
class SeeIfFormulaMakesSense(LinearTransformationScene):
    ...
    def construct(self):
        ...
        b_mat = Matrix(self.b_mat.transpose(), include_background_rectangle=True).scale(
            .75).set_column_colors(*self.fore_basis_cols).to_corner(UP+LEFT)
        a_mat = Matrix(self.a_mat.transpose(), include_background_rectangle=True).scale(
            .75).set_column_colors(*self.fore_basis_cols).next_to(b_mat)
        b_text = TextMobject("B").next_to(
            b_mat, DOWN).set_color(self.a_b_cols[1])
        a_text = TextMobject("A").next_to(
            a_mat, DOWN).set_color(self.a_b_cols[0])

        
        text = TextMobject("det(AB) = det(A).det(B)").set_width(FRAME_X_RADIUS-1).move_to(3.5*LEFT+UP).add_background_rectangle()
