...
plane = NumberPlane(
            ...
            y_axis_config={"stroke_color": BLUE_E,},
            x_axis_config={"stroke_color": YELLOW,}
        )
...
class CameraOrientations(ThreeDScene):
    def construct(self):
        ...
        tracker_groups = VGroup(
            *[VGroup(
                TextMobject(text+" ="),
                DecimalNumber(degrees(val)).scale(.8)
            ).arrange(RIGHT).set_color(col)
        ...
        for axis, label, point in zip(plane.axes, ["x", "y"], [RIGHT, UP]):
            vect = Vector(point)
            vect.add(TextMobject(label).next_to(vect, UR, buff=0))
        ...
        tracker_groups[0][-1].add_updater(lambda d: d.set_value(degrees(frame.euler_angles[0])))
        tracker_groups[1][-1].add_updater(lambda d: d.set_value(degrees(frame.euler_angles[1])))
        tracker_groups[2][-1].add_updater(lambda d: d.set_value(degrees(frame.euler_angles[2])))
