...
class LissajousCircle(Circle):
    ...
    def update_point(self, dt, speed=None):
        ...
        if self.theta > TAU:
            self.theta -= TAU
            self.cycle_incremented = True  # not a good name for what this is meant for, change.
        else: 
...
class LissajousTableScene(Scene):
    def __init__(
        ...
        **kwargs
        ...
        self.column_circle_speeds = np.linspace(*column_circle_speeds_range, self.n_cols-1)
        self.row_speed_range = row_circle_speeds_range
        self.column_speed_range = column_circle_speeds_range
        super().__init__(**kwargs)

    def setup(self):
        self.row_circles = get_circles(self.radius, self.n_rows-1, self.row_circle_speeds, self.row_buff)
        self.column_circles = get_circles(self.radius, self.n_cols-1, self.column_circle_speeds, self.column_buff, DOWN)
    ...
    def arrange_row_circles_to_match_buff(self, row_circles=None):
        ...
        x = 2*self.radius + self.row_buff +self.left_edge_buff - config["frame_width"]/2
    ...
    def arrange_column_circles_to_match_buff(self, column_circles=None):
        ...
        y = config["frame_height"]/2 - (self.top_edge_buff + 2*self.radius + self.column_buff)
    ...
    def get_horizontal_lines(self, column_circles=None, line_style=Line, **style):
        ...
        for circ in circles:
            ...
            end = np.array([config["frame_width"]/2 - self.left_edge_buff, circ.dot.get_y(), 0])
    ...
    def get_vertical_lines(self, row_circles=None, line_style=Line, **style):
        ...
        for circ in circles:
            start = circ.dot.get_center()
            end = np.array([circ.dot.get_x(), self.top_edge_buff - config["frame_height"]/2, 0])
            lines.add(line_style(start, end))
        return lines.set_style(**style)

    def add_lines_updaters(self, h_lines, v_lines):
        h_lines.add_updater(lambda h: h.become(self.get_horizontal_lines(**h_lines.get_style())))
        v_lines.add_updater(lambda v: v.become(self.get_vertical_lines(**v_lines.get_style())))
    ...
    def initiate_paths(self, **style):
        ...
        for col_circ in self.column_circles:
            for row_circ in self.row_circles:
                ...
                path = VMobject(**style).set_points_as_corners([point, point*1.0000000001])
                path.set_color(interpolate_color(row_circ.get_color(), col_circ.get_color(),0.5))
...
class DrawLissajousFigures(LissajousTableScene):
    ...
    def construct(self):
        ...
        self.play(
            AnimationGroup(
                LaggedStart(*[FadeInFrom(c, .25*LEFT) for c in self.row_circles],lag_ratio=.25),
                LaggedStart(*[FadeInFrom(c, .25*UP) for c in self.column_circles],lag_ratio=.25),
        ...
        self.play(
            AnimationGroup(
                ...
                ShowCreation(self.paths, lag_ratio=.25),
                lag_ratio=1, run_time=4
            )
        )
        self.wait(2)
...
class RadiusOne(DrawLissajousFigures):
    def __init__(self):
        super().__init__(
            radius=1, row_buff=.5, column_buff=.4, 
            top_edge_buff=.5, left_edge_buff=.5,
            )
...
class RadiusHalf(DrawLissajousFigures):
    def __init__(self):
        super().__init__(
            radius=.5, row_buff=.25, column_buff=.3, 
            top_edge_buff=.5, left_edge_buff=.5,
            )
...
class RadiusThreeFourths(DrawLissajousFigures):
    def __init__(self):
        super().__init__(
            radius=.75, row_buff=.25, column_buff=.25, 
            top_edge_buff=.5, left_edge_buff=.5,
            )
