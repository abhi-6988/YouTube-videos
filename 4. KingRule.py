...
"""
With all due respects to  MANIM, this video can be easily made
with other tools like desmos, powerpoint etc. If you are good 
at those, better use them instead. 
"""
...
class IntroScene(GraphScene):  # Do this in a video editor??
    ...
    def setup(self): # In manim, setup method will be executed before the construct method 
    ...
    def construct(self):
        ...
        exams = VGroup(
            *[Text(exam, font="Comic Sans MS") for exam in exam_texts]
        ...
        self.wait(3)
        self.play(
            LaggedStart(*[FadeInFrom(mob, UP) for mob in exams],
        ...
        properties = VGroup(
            Text("Properties Of Definite Integrals :", font="Comic Sans MS").scale(0.65),
            TexMobject("\\int_{a}^{b}f(x)dx = -\\int_{b}^{a}f(x)dx"),
            TexMobject("\\int_{a}^{b}k\ f(x)dx = k\\int_{a}^{b}f(x)dx"),
            TexMobject("\\int_{a}^{b}f(x)dx", "=",  "\\int_{a}^{b}f(a+b-x)dx"),
            TexMobject("\\int_{a}^{b}[f(x)+g(x)]dx = \\int_{a}^{b}f(x)dx + \\int_{a}^{b}g(x)dx"),
            TextMobject("...")
        ).scale(.75).arrange(DOWN, aligned_edge=LEFT, buff=0.6)
        properties[0].set_style(fill_opacity=0, stroke_width=2, stroke_color=YELLOW_D)
        king_rule = properties[3].copy()
        king_rule_text = Text(
            "King Rule :",font="Comic Sans MS",color=BLUE_E
            ).scale(0.6).next_to(king_rule, LEFT)

        self.play(LaggedStart(
            *[FadeInFrom(prop, UP) for prop in properties],
        ...
        ).arrange(DOWN,buff=0.75,aligned_edge=LEFT).next_to(king_rule_text,4*DOWN,aligned_edge=LEFT)
        ...
        fx_graph_text = TexMobject(
            "f(x)", color=MAROON_D).next_to(fx_graph,UP).scale(.75)
        fx_area = get_area(self.axes, fx_graph, 1, 7, fill_color=MAROON_D)
        fx_integral = TexMobject("\\int_{1}^{7}","f(x)","dx").move_to(fx_area).scale(0.75)
        ...
        gx_graph = graph2[1].copy().flip(
            about_point=graph2[1].point_from_proportion(number_to_proportion(4, graph2[1]))
        ).set_color(YELLOW_D)
        gx_graph_text = TexMobject(
            "f(1+7-x)", color=YELLOW_D).next_to(graph2[1], UP).scale(.75)
        gx_area = graph2[-1].copy().flip().set_fill(color=YELLOW_D)
        gx_integral = TexMobject("\\int_{1}^{7}","f(1+7-x)","dx").move_to(gx_area).scale(.75)
        ...
        flip_axis.add(TextMobject("Flip axis").scale(.75).next_to(flip_axis.get_start()))
        ...
        self.play(
            ...
            VGroup(fx_integral.copy(), TexMobject("="), gx_integral.copy()).arrange().to_edge(UP),
            run_time=2
        )
        self.wait(5)
...
class KingRule(GraphScene):
    ...
    def construct(self):
        ...
        avg = always_redraw(
            lambda: TexMobject(
                "\\frac{a+b}{2}").scale(.5).next_to(flip_axis, DOWN, 0.1)
        )

        self.play(Write(fx_text), ShowCreation(fx_graph))
        self.wait(2)
        self.play(FadeIn(fx_area), Write(VGroup(a_text,b_text)), lag_ratio=1, run_time=2)
        self.wait(2)
        ...
        gx_area = always_redraw(lambda:
            fx_area.deepcopy().flip(
                about_point=self.input_to_graph_point(
                    0.5*(a.get_value()+b.get_value()), fx_graph
                )
            ).set_style(**dict(fx_area.get_style(),**{"fill_color":YELLOW_D},)) 
        )
        gx_text = TexMobject("g(x)", color=YELLOW_D).next_to(gx_graph.get_start(),DOWN,0.1)
        gx_integral = TexMobject(
            "\\int_{a}^{b}", "g(x)", "dx").move_to(gx_area).scale(0.75)
        ...
        self.wait()
        equals = TexMobject("=").scale(0.6).next_to(fx_integral,LEFT)
        ...
        vert_line_to_g = always_redraw(
            lambda: DashedLine(
                ...
                gx_graph.point_from_proportion(number_to_proportion(8-x.get_value(),gx_graph)),
                color = YELLOW_D
            )
        )
        ...
        fg_line = always_redraw(
            lambda: DashedLine(
                vert_line_to_g.get_end(),
                vert_line_to_f.get_end()
            ).set_color_by_gradient(YELLOW_D, MAROON_D)
        )
        x_text = always_redraw(
            lambda: VGroup(
                Dot(self.x_axis.number_to_point(x.get_value())).scale(.7),
                TexMobject("x").scale(.7).next_to(
                self.x_axis.number_to_point(x.get_value()),DOWN+LEFT,0.1
            )).set_color(YELLOW_D)
        )
        x_prime_text = always_redraw(
            lambda: VGroup(
                Dot(self.x_axis.number_to_point(8-x.get_value())).scale(.7),
                TexMobject("{x}'").scale(.7).next_to(
        ...
        self.play(Write(x_text), Write(x_prime_text),)
        self.wait()
        ...
        dist_arrow = Vector(
            LEFT*2*self.x_axis.unit_size).shift(self.x_axis.number_to_point(4)
            ).shift(2*UP)
        self.play(GrowFromPoint(dist_arrow, dist_arrow.get_start()))
        self.play(Rotating(dist_arrow,radians=PI, axis=IN, about_point=dist_arrow.get_start()),
            run_time=2
        )
        self.wait()
        ...
        input_avg = TexMobject("\\frac{x + {x}'}{2}").scale(0.5)
        input_avg[0][0].set_color(YELLOW_D)
        input_avg[0][2:4].set_color(MAROON_D)
        avg_copy = avg.copy()
        average = VGroup(input_avg, TexMobject("=").scale(.5), avg_copy).scale(2).arrange().to_edge(RIGHT)
        relation = VGroup(
            ...
            TexMobject("-"),
            average[0][0][0].copy()
            ).arrange().next_to(average,DOWN,1,aligned_edge=LEFT)
        
        
        self.play(Write(average), run_time=3)
        self.wait(2)
        self.play(FadeInFrom(relation, UP))
        self.wait()
        self.play(
            relation.shift, 2*UP,
            FadeOut(average)
        ...
        self.wait()
        self.play(
            x.set_value, 8, rate_func=there_and_back, run_time=8
        )
        self.wait(3)

        g_as_f = TexMobject("g(x)","=","f","(a+b-","x",")").next_to(relation,DOWN,1,aligned_edge=RIGHT)
        ...
        abx_integral = VGroup(
            TexMobject("="),
            TexMobject("\\int_{a} ^ {b}"),
            VGroup(g_as_f[2:].copy(),
            TexMobject("dx")).arrange(buff=0.1)
        ).scale(.75).arrange().next_to(fx_integral)
...
#         self.camera.init_background()
        ...
        question = VGroup(
            ...
            TextMobject("  (JEE Mains - 2018)")
        ).scale(.65).arrange().to_corner(UL).set_color(YELLOW_D)

        i1 = VGroup(
            TextMobject("I ="),
            question[1].copy().set_color(WHITE),
        ).arrange().next_to(question, DOWN, 0.6, aligned_edge=LEFT)
        eq1 = TexMobject("\\rightarrow 1").next_to(i1,buff=1)

        fx_exp = VGroup(
            TexMobject("f(x) ="),
            i1[1].copy()
        ).arrange().next_to(i1, DOWN, aligned_edge=LEFT)

        king_rule = VGroup(
            TextMobject("King rule :", color=MAROON_D),
            TexMobject("\\int_{a}^{b}f(x)dx", "=",
                       "\\int_{a}^{b}f(a+b-x)dx").scale(0.65)
        ).arrange().next_to(eq1,buff=1)
        ...
        ).arrange().next_to(i1,2*DOWN, aligned_edge=LEFT)
        i_int2 = VGroup(
            TexMobject("="),
            TexMobject(
                "\\int_{-\\frac{\pi}{2}} ^ {\ \\frac{\pi}{2}}","\\frac{2^{x}sin ^ {2}x}{1+2 ^ {x}}dx"
            ).scale(.65)
            ).arrange().next_to(i_int1[0][0][1],3*DOWN,aligned_edge=LEFT)
        ...
        self.wait()
        self.play(FadeOut(i_int1[1]),FadeOut(i_int2[0]),i_int2[1].next_to,i_int1[0],)
        self.wait(7)
        self.play(Indicate(i_int2[1][1][:2], scale_factor=1.5))
        self.wait(2)
        
        # i2 = VGroup(i_int1[0],i_int2[1])
        eq2 = TexMobject("\\rightarrow 2").next_to(i_int2[1])
        ...
        two_i = VGroup(
            ...
            TexMobject("+"),
            i_int2[1].copy()   
        ).arrange().next_to(i_int1, 2*DOWN, aligned_edge=LEFT)
        ...
        ).arrange().next_to(two_i)
        simplified_2i = VGroup(
            TexMobject("="),
            TexMobject(
                "\\int_{-\\frac{\pi}{2}} ^ {\ \\frac{\pi}{2}}",
                "\\sin^{2}x}","dx"
            ).scale(.65)
        ).arrange().next_to(clubbed_2i,1.5*DOWN, aligned_edge=LEFT)
        ...
        self.play(
            FadeOut(clubbed_2i),
            simplified_2i.next_to, two_i 
        )
        self.wait()
        self.play(
            FadeOut(two_i[1:]),FadeOut(two_i[0][1]),
            simplified_2i.next_to, two_i[0][0],
            run_time=2
            )
        self.wait()

        two_i_val = TexMobject("=","\\frac{\pi}{2}").scale(.65).next_to(simplified_2i)
        i_val = VGroup(
            TexMobject("I ="),
            TexMobject("\\frac{\pi}{4}").scale(.65)
        ).arrange().next_to(two_i, 1.5*DOWN, aligned_edge=LEFT)
        i_val.add_background_rectangle(
            stroke_color=YELLOW_D, stroke_width=2, stroke_opacity=1,
            buff=0.25
            )
